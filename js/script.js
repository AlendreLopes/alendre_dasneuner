...
(function () {
	...
	var userAgent = navigator.userAgent.toLowerCase(),
		...
		plugins = {
			...
			preloader: $(".preloader"),
			boxSocial: $(".box-project-social"),
			floatWhats: $("#floatWhatsApp"),
			captcha: $('.recaptcha'),
	...
	$window.on('load', function () {
		...
		if (plugins.preloader.length && !isNoviBuilder) {
			pageTransition({
				...
				onReady: function () {
					...
					windowReady = true;
  					plugins.boxSocial.css({"display":"inline-block"});
  					/*plugins.floatWhats.css({"display":"inline-block"});*/
  				}
			});
		}
		/**
		...
		if (plugins.isotope.length) {
			...
			//
			var resizeTimout, categoryChoose;
			//portfolio
			$("[data-isotope-filter]").on("click", function (e) {
				...
				var iso = $('.isotope[data-isotope-group="' + this.getAttribute("data-isotope-group") + '"]'),
					isotopeAttrs = {
						...
						filter: this.getAttribute("data-isotope-filter") === '*' ? categoryChoose : '[data-filter*="' + this.getAttribute("data-isotope-filter") + '"]'
					};
				//
		...
		if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
			$(".whatsapp").attr({
				"href" : "https://api.whatsapp.com/send?phone=554131563495",
				"title": ""
			});
    	} else{
			$(".whatsapp").attr({
				"href"  : "https://web.whatsapp.com/send?phone=554131563495",
				"title" : "",
			 	"target": "_blank"
			});
    	}
	...
	$(function () {
		...
		$(".services").on('click', function(e){
			...
			servicesGroup  = this.getAttribute("data-services-group"),
			servicesFilter = this.getAttribute("data-services-filter");
			switch(servicesId){
				case '1':
					$('#all').removeClass('active');
					$('#projects').addClass('active');
					$('#regularize').removeClass('active');
					$('#works').removeClass('active');
					$('#several').removeClass('active');
					servicesRequested(servicesGroup, servicesFilter);
				break;
				case '2':
					$('#all').removeClass('active');
					$('#projects').removeClass('active');
					$('#regularize').addClass('active');
					$('#works').removeClass('active');
					$('#several').removeClass('active');
					servicesRequested(servicesGroup, servicesFilter);
				break;
				...
				case '4':
					...
					$('#regularize').removeClass('active');
					$('#works').removeClass('active');
					$('#several').addClass('active');
					servicesRequested(servicesGroup, servicesFilter);
				break;
			}
		});
		// IsotopeServices
		function servicesRequested(requestedGroup, requestedFilter) {
			if (plugins.isotope.length) {
				var isogroup = [];
				for (var i = 0; i < plugins.isotope.length; i++) {
					var isotopeItem = plugins.isotope[i],
				...
				//filter.parents(".isotope-filters").find('.active').removeClass("active");
				//filter.addClass("active");
				// filter: this.getAttribute("data-services-filter") === '*' ? '*' : '[data-filter*="' + requestedFilter + '"]'
				var iso = $('.isotope[data-isotope-group="' + requestedGroup + '"]'),
					isotopeAttrs = {
						itemSelector: '.isotope-item',
						layoutMode: iso.attr('data-isotope-layout') ? iso.attr('data-isotope-layout') : 'masonry',
						filter: '[data-filter*="' + requestedFilter + '"]'
					};
				if (iso.attr('data-column-width')) {
					isotopeAttrs.masonry = {
						columnWidth: parseFloat(iso.attr('data-column-width'))
		...
		function attachFormValidator(elements) {
			...
			regula.custom({
				name: 'PhoneNumber',
				defaultMessage: 'Número inválido',
				validator: function() {
		...
		if (plugins.captcha.length) {
			$.getScript("//www.google.com/recaptcha/api.js?onload=onloadCaptchaCallback&render=explicit&hl=pt-BR");
		}
		...
		if (plugins.rdMailForm.length) {
			var i, j, k,
				msg = {
					'MF000': 'E-mail enviado com sucesso!',
					'MF001': 'Destinatário não foi configurado!',
					'MF002': 'Formulário não funciona localmente!',
					'MF003': 'Por favor, defina o campo de e-mail no seu formulário!',
					'MF004': 'Por favor, defina o tipo do seu formulário!',
					'MF254': 'Algo está errado como PHPMailer!',
					'MF255': 'Algo está errado.'
				};
